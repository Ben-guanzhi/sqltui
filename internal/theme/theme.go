...
func New(p Palette) *Theme {
	...
	// ColCursor: soft warm tint on the focused column header — visible but unobtrusive.
	t.ColCursor = lipgloss.NewStyle().Foreground(lipgloss.Color("#1a1510")).Background(lipgloss.Color("#d4c5a9")).Bold(true)
	t.Gutter = lipgloss.NewStyle().Foreground(fgDim)
	t.Border = lipgloss.NewStyle().Foreground(fgDim)
	t.Match = lipgloss.NewStyle().Foreground(lipgloss.Color(p.Highlight)).Bold(true).Underline(true)
