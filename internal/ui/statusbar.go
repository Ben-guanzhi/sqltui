...
type statusInfo struct {
	...
	TimeRange  string   // time range for queries ("" hides it)
	Page       int      // 1-based page number (0 hides it)
	PageSize   int      // rows per page (0 hides it)
}
...
func renderStatusBar(info statusInfo, width int, th *theme.Theme) string {
	...
	if info.Col != "" {
		left = append(left, sbSeg{text: tag(4, "col: "+info.Col), drop: 4})
	}
	if info.Mode != "" {
		left = append(left, sbSeg{text: tag(5, "["+info.Mode+"]"), drop: 5})
	}

	// Pagination and time range go to the right side.
	// When PagerMode is active, page info is rendered with a highlighted style.
	var right []sbSeg
	if info.TimeRange != "" {
		right = append(right, sbSeg{text: th.StatusBar.Render(" " + info.TimeRange + " "), drop: 7})
	}
	if info.Page > 0 {
		var pageText string
		if info.PageSize > 0 {
			pageText = fmt.Sprintf(" p%d / %d rows ", info.Page, info.PageSize)
		} else {
			pageText = fmt.Sprintf(" p%d ", info.Page)
		}
		right = append(right, sbSeg{text: th.StatusBar.Render(pageText), drop: 6})
	}
	if info.Conn != "" {
