...
func Run(kind string) error {
	switch kind {
	case KindMysql, KindPostgres, KindRedis, KindOpenObserve, KindSqlServer, KindOracle:
	default:
...
var (
	...
	// cachedOobeSchema is the singleton schema overlay for OpenObserve.
	// Reusing it preserves cursor/offset when the user navigates back from a table.
	cachedOobeSchema ui.Overlay
)
...
func RegisterFactories(kind string) {
	...
	if kind == KindOpenObserve {
		ui.Factories["schema"] = func(ctx ui.AppContext, arg string) (ui.Overlay, error) {
			// Reuse cached instance to preserve cursor position across navigations.
			if cachedOobeSchema != nil {
				return cachedOobeSchema, nil
			}
			ov := popup.NewOobeSchemaOverlay(ctx)
			if ov == nil {
				if origSchema != nil {
					return origSchema(ctx, arg)
				}
				return nil, fmt.Errorf("schema: no oobe connection")
			}
			cachedOobeSchema = ov
			return ov, nil
		}
