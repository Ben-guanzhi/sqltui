...
func TestRenderSheetEdgeFollow(t *testing.T) {
	...
	visible := height - 3 // row header + col header + footer hint
	matched := sheetMatchedCols(f, "")
	dataRow := 0
	curStart := sheetBodyLineOffsetCols(f, matched, lastField, dataRow, keyW, valW)
	curEnd := curStart + sheetBodyEntryHeightCols(f, matched, lastField, dataRow, keyW, valW) - 1
